...
export default function SelectVariant({ initialColor }: { initialColor: string }) {
  const { product, selectedVariant, setSelectedVariant } = useProductStore();

  useEffect(() => {
    if (product?.variants) {
      const variant = product.variants.find((v) => v.color === initialColor) || null;
  ...
  const hasVariants = nonNullProduct.variants && nonNullProduct.variants.length > 0;

  const availableColors: string[] = hasVariants
    ? [
        ...new Set(
          nonNullProduct.variants!.map((variant) => variant.color).filter(Boolean)
        ),
      ]
  ...
  const handleColorChange = (color: string) => {
    const variant = nonNullProduct.variants?.find((v) => v.color === color) || null;
    setSelectedVariant(variant);
